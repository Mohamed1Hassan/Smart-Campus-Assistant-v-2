...
export default function Schedule() {
  ...
  const [currentDay, setCurrentDay] = useState("Monday"); // Server-safe initial value
  const [filters, setFilters] = useState({ day: "Days", status: "Status" }); // Server-safe initial value
  const [searchTerm, setSearchTerm] = useState("");
  const [hasMounted, setHasMounted] = useState(false);
  const { success, info } = useToast();

  useEffect(() => {
    // Defer the hasMounted flag update to avoid the sync React warning
  ...
  const {
    ...
    queryFn: async () => {
      ...
      if (response.success && response.data) {
        ...
        return (response.data as BackendScheduleItem[]).map((item) => {
  ...
  // Save filters when they change, but only after initial load from localStorage
  useEffect(() => {
    if (hasMounted) {
