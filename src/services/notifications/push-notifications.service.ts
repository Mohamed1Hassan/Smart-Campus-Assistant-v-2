...
export class PushNotificationsService implements PushNotificationService {
  ...
  private webPush: typeof webpush | null = null; // Web push library instance
  ...
  public async sendPushNotification(
    ...
    try {
      ...
      if (
        pushMessage.priority?.level === "HIGH" ||
        pushMessage.priority?.level === "CRITICAL"
      ) {
  ...
  public async getPushStatus(messageId: string): Promise<PushDelivery> {
    ...
    return (
      ...
      this.createPushDelivery({} as NotificationMessage)
    );
  }
  ...
  public async schedule(
    ...
    this.logInfo("Push notification scheduled", {
      messageId: message.id,
      scheduledAt,
    });
  }
  ...
  private async processPush(
    ...
    try {
      ...
      this.logInfo("Push notification delivered successfully", {
        messageId: message.id,
      });
    } catch (error) {
      ...
      this.logError("Push notification delivery failed", error, {
        messageId: message.id,
      });
    }
  ...
  private initializeWebPush(): void {
    try {
      ...
      this.logInfo("Web push initialized", {
        vapidSubject: this.pushConfig.vapid.subject,
      });
    } catch (error) {
  ...
  private async processPushQueue(): Promise<void> {
    ...
    for (const push of pendingPushes.slice(0, 10)) {
      // Process up to 10 pushes at a time
      try {
