...
export class EmailNotificationsService implements EmailNotificationService {
  ...
  public async sendEmail(
    ...
    try {
      ...
      if (
        emailMessage.priority?.level === "HIGH" ||
        emailMessage.priority?.level === "CRITICAL"
      ) {
  ...
  public async getEmailStatus(messageId: string): Promise<EmailDelivery> {
    ...
    return (
      ...
      this.createEmailDelivery({} as NotificationMessage)
    );
  }
    ...
    try {
      const emailOptions: nodemailer.SendMailOptions = {
        ...
        attachments: this.processAttachments(
          (message.data?.attachments as EmailAttachment[]) || [],
        ),
      };
      ...
      throw new Error(
        `SMTP delivery failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  private async processEmailQueue(): Promise<void> {
    ...
    for (const email of pendingEmails.slice(0, 5)) {
      // Process up to 5 emails at a time
      try {
        await this.processEmail(email);
      } catch (error) {
        this.logError("Email processing failed", error, {
          messageId: email.id,
        });
      }
    }
